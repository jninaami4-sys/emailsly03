...
final class LeadsAdmin {
  public function index(): void {
    Auth::requireAdmin();
    Response::json(['rows' => Database::pdo()->query('SELECT * FROM contact_leads ORDER BY created_at DESC LIMIT 500')->fetchAll()]);
  }
  public function update(array $p): void {
    ...
    $vals[] = $p['id'];
    Database::pdo()->prepare('UPDATE contact_leads SET ' . implode(',', $fields) . ' WHERE id = ?')->execute($vals);
