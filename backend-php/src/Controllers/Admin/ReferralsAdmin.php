...
final class ReferralsAdmin {
  public function index(): void {
    Auth::requireAdmin();
    Response::json(['rows' => Database::pdo()->query('SELECT * FROM referrals ORDER BY created_at DESC LIMIT 500')->fetchAll()]);
  }
  public function update(array $p): void {
    ...
    $vals[] = $p['id'];
    Database::pdo()->prepare('UPDATE referrals SET ' . implode(',', $fields) . ' WHERE id = ?')->execute($vals);
