...
export const getMyReferralSummary = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<ReferralSummary> => {
    ...
    const [{ data: profile }, { data: ledger }, { data: refs }, { data: bal }] = await Promise.all([
...
export const getMyReferralBalance = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<{ balance_cents: number; currency: string }> => {
    ...
    return { balance_cents: typeof data === "number" ? data : 0, currency: "USD" };
  });
...
export const attachReferrer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: current } = await sb
      ...
      .eq("user_id", context.userId)
      .maybeSingle();
    if (current?.referred_by_user_id) return { ok: false, reason: "Already attributed" };
    if (current?.referral_code === code) return { ok: false, reason: "Cannot use your own code" };
    ...
    const { data: refProfile } = await admin
      ...
      .maybeSingle();
    if (!refProfile) return { ok: false, reason: "Code not found" };
    if (refProfile.user_id === context.userId) return { ok: false, reason: "Cannot use your own code" };
